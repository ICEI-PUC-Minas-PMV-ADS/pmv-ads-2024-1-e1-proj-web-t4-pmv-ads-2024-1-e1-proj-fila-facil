...
function createPriceAndQuantityDiv(prato) {
  ...
  quantityText.setAttribute("data-id-prato", prato.idPrato);
  ...
  minusButton.setAttribute("onclick", "removeItem(this)");
  ...
  plusButton.setAttribute("onclick", "adicionaItem(this)");
...
function renderCardapio(categories, restaurant, pratoSearch = null) {
  ...
  categories.forEach((category, index) => {
    ...
    cardapioTabSection.setAttribute(
      "aria-labelledby",
      `pills-${formattedCategory}-tab`
    );
    ...
    if (pratoSearch) {
      ...
      pratos = restaurant.cardapio.filter(
        (prato) =>
          prato.categoriaPrato === category &&
          (prato.nomePrato.toLowerCase().includes(pratoSearch) ||
            prato.descricaoPrato.toLowerCase().includes(pratoSearch))
      );
    } else {
      pratos = restaurant.cardapio.filter(
        (prato) => prato.categoriaPrato === category
      );
    }
