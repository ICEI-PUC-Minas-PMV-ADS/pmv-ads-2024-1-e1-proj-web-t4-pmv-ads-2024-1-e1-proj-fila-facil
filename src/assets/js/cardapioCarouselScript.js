 ...
 */
function handleSearchForm(searchForm, baseUrl, id) {
  searchForm.addEventListener("submit", function (event) {
    event.preventDefault();
    const searchQuery = document.getElementById("search-input").value;
    const url = `${baseUrl}?id=${id}&query=${encodeURIComponent(
      searchQuery
    ).toLowerCase()}`;
 ...
 */
function handleRestauranteRendering(searchParam, id, restaurantes) {
  if (id) {
    const restauranteObj = restaurantes.find(
      (restaurante) => restaurante.idRestaurante == id
    );
    if (restauranteObj) {
      ...
      document.querySelector(".restaurante-nome").textContent =
        restauranteObj.nomeRestaurante;
    } else {
 ...
 */
function handleSearchNotification(baseUrl, searchParam, id) {
  if (searchParam) {
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  handleSearchForm(searchForm, baseUrl, id);
  handleRestauranteRendering(searchParam, id, restaurantes);
  handleSearchNotification(baseUrl, searchParam, id);
  checkForErrorSearch();
