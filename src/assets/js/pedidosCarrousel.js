...
carousels.forEach(function(carousel) {
    if (window.matchMedia("(min-width:576px)").matches) {
        var cardWidth = carousel.querySelector('.carousel-item');

        // Checa se o carrossel está vazio
        if (cardWidth === null) {
            carousel.parentElement.querySelector('.carousel-control-next').style.display = 'none';
            carousel.parentElement.querySelector('.carousel-control-prev').style.display = 'none';
            carousel.parentElement.innerHTML = '<h3 class="text-center" style="margin-top: 20vh;">Parece que não tem nada para ser exibido aqui. Faça um novo pedido</h3>';
            return
        } else {
            cardWidth = cardWidth.offsetWidth;
        }
